...
namespace TMNT.Controllers {
    ...
    public class HomeController : Controller {
        ...
        public ActionResult Index() {
            ...
            var expiringItems = new InventoryItemRepository(DbContextSingleton.Instance).Get().Where(item => item.ExpiryDate < DateTime.Today.AddDays(30) && !(item.ExpiryDate < DateTime.Today) && item.Department == userDepartment);
            var expiredItems = new InventoryItemRepository(DbContextSingleton.Instance).Get().Where(item => item.ExpiryDate < DateTime.Today && item.Department == userDepartment);
